...
extern "C" void ippe_solve_square(float squareLength, double* _imagePoints, double* _cameraMatrix, double* _distCoeffs, double* _rvec1, double* _tvec1, float* reprojErr1, double* _rvec2, double* _tvec2, float* reprojErr2) {
    ...
    cv::Mat r1_out = cv::Mat(3, 3, CV_64FC1, _rvec1);
    cv::Mat t1 = cv::Mat(3, 1, CV_64FC1, _tvec1);
    cv::Mat r2 = cv::Mat(3, 1, CV_64FC1);
    cv::Mat r2_out = cv::Mat(3, 3, CV_64FC1, _rvec2);
    cv::Mat t2 = cv::Mat(3, 1, CV_64FC1, _tvec2);
    IPPE::PoseSolver solver = IPPE::PoseSolver();
    float err1, err2;
    solver.solveSquare(squareLength, img, cam, dist, r1, t1, err1, r2, t2, err2);

    *reprojErr1 = err1;
    *reprojErr2 = err2;
    cv::Rodrigues(r1, r1_out);
    cv::Rodrigues(r2, r2_out);
}
